...
def build_it_response_figure(result, *, include_hover_metadata=False):
    ...
    width = group_figure_width(tuple(event.name for event in events), base=6.5)
    ...
    for event_index, event in enumerate(events):
        ...
        for group_index, group in enumerate(groups):
            rows = [row for item in result.files for row in item.responses
                    if item.group == group and row.event_id == event.event_id and row.status == "ok"]
            ...
            for row, value, delta_x in zip(rows, values, jitter):
                point = ITResponseScatterPoint(centre + float(delta_x), float(value), row.sample_id,
                                               group, event.event_id, event.name)
        ...
        series.append(ITResponseScatterSeries(event.event_id, event.name, tuple(event_points)))
    configure_group_ticks(axis, tuple(event.name for event in events))
